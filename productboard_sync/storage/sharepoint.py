...
class SharePointStorageBackend(GraphAuthMixin, StorageBackend):
    def __init__(
        self,
        tenant_id: str,
        client_id: str,
    ...
    def _item_url(self, path: str) -> str:
        return (
            f"{GRAPH_BASE}/sites/{self._site_id}/drives/{self._drive_id}"
            f"/items/{self._folder_id}:/{path}:/content"
        )
    ...
    def list_files(self, prefix: str = "") -> list[str]:
        url = f"{GRAPH_BASE}/sites/{self._site_id}/drives/{self._drive_id}/items/{self._folder_id}/children"
        items = self._graph_list_children(url)
        names = [item["name"] for item in items if "name" in item]
        return [name for name in names if name.startswith(prefix)]

    def delete_file(self, path: str) -> None:
        url = f"{GRAPH_BASE}/sites/{self._site_id}/drives/{self._drive_id}/items/{self._folder_id}:/{path}:"
